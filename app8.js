...
// これより下はBBS関係
// 投稿のチェック機能（掲示板の投稿数を返す）
app.post("/check", (req, res) => {
  // 本来はここでDBMSに問い合わせる
  res.json({ number: bbs.length });
});

// 投稿の読み込み機能（掲示板の投稿を読み込む）
...
app.post("/post", (req, res) => {
  ...
  const userId = req.body.userId; // userIdを受け取る
  const name = req.body.name;
  const message = req.body.message;
  console.log([userId, name, message]);

  // 一意なIDを生成（投稿IDとして使用）
  const postId = Date.now(); // 現在時刻をIDとして使用

  // 本来はここでDBMSに保存する
  bbs.push({ userId: userId, name: name, message: message, id: postId });
  res.json({ number: bbs.length });
});
